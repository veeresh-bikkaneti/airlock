...
| **`Uninstall‑AI.ps1`** | • Checks the current shell for a leftover platform redirect first, via `ai-doctor`, *before* removing anything.<br>• Removes `~/.ai-platform` (everything `setup.ps1` deployed) and `~/.airlock-src` (`install.ps1`'s clone target).<br>• Removes the `profile-helpers.ps1` sourcing line from `$PROFILE`.<br>• Removes this platform's firewall rules.<br>• `-WhatIf` previews every action with nothing touched. | The missing reverse of the `irm \| iex` installer. Deliberately leaves Ollama and every pulled model alone — not the uninstaller's business to delete gigabytes of model weights as a side effect of removing PowerShell scripts. |
| **`profile‑helpers.ps1`** | • Provides the **`ai‑*` command suite** that users source from their PowerShell profile.<br>• Wraps the underlying scripts to set environment variables automatically.<br>• Implements convenience helpers: `ai‑port`, `ai‑provider`, `ai‑switch`, `ai‑code`, `ai‑claude‑on`/`ai‑claude‑off`, `ai‑doctor`, `ai‑hermes‑start/stop`, `ai‑memory‑start/stop/status/on/off`, `ai‑audit‑last`, `ai‑policy`, `ai‑models`, `ai‑auth`, `ai‑cache`, `ai‑config`, `ai‑health`.<br>• Blocks direct `ollama serve` (users must use `ai‑start`). | Gives a **single, beginner‑friendly CLI**. Prevents users from accidentally bypassing the security/shutdown logic in `Start‑AI.ps1`. `ai‑claude‑on`/`ai‑doctor` specifically close the class of failure in `docs/adr/ADR-005-model-sizing-against-vram.md`'s sibling incident (`docs/bugs/PBI-airlock-anthropic-base-url.md`): a permanent, unguarded redirect that outlives the platform. |
