...
# EPIC E — Retrieval governance and capability enforcement

*Added 2026-08-12 after a coverage audit. The plan as written contained zero references to LangChain, vector storage, embeddings, or retrieval, despite `memory-service` being an active component that sits in the request path. It also treated model capability as advisory metadata rather than an enforced gate. Both are direct contributors to the reported symptom — "the local model hallucinates and doesn't help at all."*

**Why this epic exists.** Hallucination in this system has three independent sources, and the plan previously addressed only the first:

1. **Wrong model, wrong speed** — AIR-6. A CPU-spilled 30B looks broken.
2. **Bad retrieval** — nothing. `ai-memory-on` proxies every client request through `memory-service`, which injects retrieved context. Retrieved context that is stale, irrelevant, or unattributed is *worse than no context*: the model states it confidently because it arrived as authoritative input.
3. **Model below the capability floor** — AIR-14, but advisory only. A model that cannot emit reliable tool calls will hallucinate tool results instead of admitting failure.

---

## AIR-17 — Retrieval path governance

**Problem.** `ai-memory-on` rewrites the active endpoint so all client traffic proxies through `memory-service`, which performs RAG over a vector store using `nomic-embed-text` embeddings. Nothing in the platform governs:

- whether the index is **fresh** relative to the repository it describes,
- whether retrieved chunks are **relevant** to the query,
- whether injected context is **attributed** so the model can distinguish retrieved material from its own priors,
- what happens when retrieval **fails** — silent degradation to an unaugmented model, indistinguishable from success.

A stale index is the highest-severity case. Retrieval that returns a function signature deleted three commits ago hands the model a confident, wrong fact. The model then builds on it. This is a *manufactured* hallucination, and the platform manufactured it.

The dependency surface is also undeclared: `memory-service/.venv` contains `langchain_core` and `langsmith`, neither documented, neither pinned in any manifest visible at the platform level, and `langsmith` is a telemetry client that may transmit externally.

**Acceptance criteria.**

- [ ] `memory-service` declares its retrieval stack explicitly in `config/memory-service.json`: embedding model, vector store implementation, index path, chunking strategy, `topK`, and similarity threshold. No implicit defaults.
- [ ] All Python dependencies are pinned in a committed manifest. `langchain`, `langchain_core`, and `langsmith` versions are recorded, since a minor LangChain release can silently change chunking or retriever behaviour.
- [ ] **`langsmith` telemetry is disabled by default.** If tracing is wanted it is explicit opt-in, and the platform states what leaves the machine. An air-gapped-by-design platform must not ship a default-on external tracer.
- [ ] Index freshness is tracked: every index records the commit SHA and timestamp it was built against. `ai-memory-status` reports staleness in commits and hours.
- [ ] **Retrieval is refused, not degraded, when the index is stale beyond a configured threshold.** Default: refuse above 50 changed files or 7 days. A stale-index warning that the user can ignore is not sufficient — refusal is the correct default because the failure mode is silent and confident.
- [ ] Every injected chunk carries provenance in the prompt: source path, line range, and index build time. The model must be able to say "per `src/api.py:42-58`, indexed 3 days ago" rather than asserting it as ambient truth.
- [ ] A similarity floor is enforced. Chunks below threshold are dropped rather than padded to `topK`. Returning the least-bad match when nothing is relevant is a primary hallucination source.
- [ ] Retrieval failure is **loud**. If `memory-service` is unreachable or returns nothing, the client is told the response is unaugmented. Silent fallback to a bare model is prohibited.
- [ ] `ai-memory-on` verifies index freshness before routing, and refuses on a stale index with the reindex command.
- [ ] `ai-doctor` reports memory-service state: running, index age, document count, embedding model, and whether clients are currently routed through it.
- [ ] A retrieval eval exists: a fixed set of repo questions with known-correct source locations, scored on whether the right chunk was retrieved. Run in CI. Retrieval quality is otherwise unfalsifiable and will drift silently.

**Dependencies.** AIR-1 and AIR-2 — `ai-memory-on` is an endpoint mutation and must go through the ledger and liveness guard like any other. R-8 already covers `directEndpoint` restoration.

**Note on overlap.** Three retrieval systems are now installed on this machine: `memory-service` (embedding-based RAG), `tokensave` (semantic code graph, 34 languages), and Headroom (context compression with its own memory store). They have no defined precedence and may inject overlapping or contradictory context into the same request. AIR-15's harness compatibility matrix should state which retrieval layer is authoritative per harness, or the layers should be made mutually exclusive.

---

## AIR-18 — Capability floor enforcement

**Problem.** AIR-14 adds `toolCalling` metadata and warns when a model falls short. A warning does not prevent the failure it describes — the user proceeds, the model fabricates tool results, and the platform's own metadata predicted it.

A model that cannot reliably emit structured tool calls does not fail cleanly inside an agentic harness. It produces plausible prose *describing* a tool call, or invents the result of one. Both read as hallucination and neither is recoverable by the harness.

**Acceptance criteria.**

- [ ] `models.json` gains `capabilityFloor` per **harness**, not per model — the requirement belongs to the consumer. Example: `aider` requires `toolCalling: partial`; Claude Code requires `reliable`.
- [ ] Launching a harness with a model below its floor **fails by default** with a message naming the model, the harness, the specific missing capability, and the models on this machine that do meet it.
- [ ] `-Force` permits the launch after an explicit acknowledgement that output reliability is degraded.
- [ ] `toolCalling` values are populated by a **measured probe**, not vendor claims: issue N structured tool-call requests, score well-formed responses, and record the pass rate with its date. Vendor capability claims and observed behaviour diverge routinely at this tier.
- [ ] `ai-models` displays the measured pass rate alongside each model, and which harnesses each model clears.
- [ ] A capability probe runs after any `ollama pull` so metadata cannot go stale against the installed model set.
- [ ] The context window is enforced as part of the floor. A model whose context cannot hold the harness system prompt plus a working file set is below floor for that harness regardless of its tool-calling score — Claude Code's system prompt alone is substantial.

**Rationale for failing closed.** This is the one place the platform should be opinionated. The cost of a refused launch is thirty seconds and a clear message. The cost of an accepted launch below floor is a session of fabricated tool results that the user may act on. Those costs are not symmetric.
...
| # | Test | Assertion |
|---|---|---|
| R-15 | Index built at commit A; repo advanced 60 files; run `ai-memory-on` | Refuses; names staleness and the reindex command |
| R-16 | Query with no chunk above the similarity floor | Zero chunks injected; response marked unaugmented |
| R-17 | Stop `memory-service`, then issue a client request | Client is told the response is unaugmented — no silent fallback |
| R-18 | Inspect any injected context block | Every chunk carries path, line range, and index build time |
| R-19 | Launch Claude Code with a model whose `toolCalling` is `partial` | Fails; names the gap and the qualifying models; `-Force` overrides |
| R-20 | Fresh `ollama pull`, then read `models.json` | Capability probe has run; `toolCalling` and pass-rate date populated |
| R-21 | Inspect `memory-service` process environment | `langsmith` tracing disabled unless explicitly opted in |
| R-22 | Retrieval eval corpus | Pass rate at or above the committed baseline; CI fails on regression |

---

## Sequencing note

AIR-18 belongs in the **critical path**, ahead of AIR-14. The revised path is:

```
AIR-6 → AIR-7 → AIR-2 → AIR-10 → AIR-18
```

AIR-6 and AIR-7 make the local backend *reachable and fast*. AIR-18 is what stops it being reached with a model that will fabricate. Shipping 6 and 7 without 18 makes a fast path to unreliable output.

AIR-17 can follow, but `ai-memory-on` should be **disabled by default** until it lands. An ungoverned retrieval layer in the request path is a net negative against an unaugmented model — the unaugmented model at least does not receive confident wrong facts as input.
