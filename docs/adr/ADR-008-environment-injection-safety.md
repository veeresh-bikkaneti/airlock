...
`grep -rl uninstall --include=*.ps1` returns nothing. `install.ps1` is a one-line `irm | iex` with no reverse. A user who wants out edits `$PROFILE` by hand and guesses at `~/.ai-platform`. Separately, any uninstall must warn that **running processes keep a frozen copy of the environment** — the Gap 1 problem in its uninstall form.
