...
**Rejected: generating a video.** A rendered mp4 cannot be inspected, filtered, or regenerated from live audit logs, and the repo already carries a 5.5 MB `brag.mp4` that GitHub will not play inline — the README says so itself. Interactive scenes strictly dominate.
