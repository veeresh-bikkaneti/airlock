...
That's the tool call as raw text, never executed. Identical failure shape to `qwen2.5-coder:7b`'s transcript in the opencode section below. Two independent harnesses, same Ollama model, same symptom.

**Correction, 2026-08-20:** this was originally blamed on "Ollama's OpenAI-compat tool-calling layer." Re-tested directly against Ollama's *native* `/api/chat` (no OpenAI-compat translation at all) and got the identical failure, so that explanation was wrong. `qwen2.5-coder:7b`'s own chat template genuinely defines `<tool_call>` markup and Ollama's `/api/tags` lists `"tools"` in its capabilities — the template supports tool calls syntactically. The model's weights just aren't reliably trained to consistently emit that exact wrapper, so it falls back to printing its best-guess JSON as plain text. Picking a model that's actually reliable at this (`qwen3-coder:30b` passed 3/3 on the same single-tool-call test where this model failed 3/3) fixes it more directly than any proxy — see the opencode section below and `README.md`'s "Model choice for tool-calling" section for the full finding, including an open caveat that even the better model isn't proven reliable across a full multi-turn tool surface yet.

**What actually works reliably regardless of model: attach the file yourself instead of hoping the model asks for it.**
...
**Pick a model with Ollama's "Tools" badge before anything else — this matters more than the port.** `qwen2.5-coder:7b` reproducibly fails opencode's tool-calling (see the reproduced failure below); `qwen3-coder:30b` and `devstral-small-2:24b` don't, on the same single-tool-call test. Check `ollama.com/library/<model>` for the badge, or `ollama list`/`/api/tags` for `"tools"` in capabilities as a rougher signal. The template points `provider.ollama.options.baseURL` straight at Ollama (`http://127.0.0.1:12345/v1` — use whatever `ai-port` shows you) and defaults `model` to `ollama/devstral-small-2:24b`. Edit the model line if that's not what you have pulled, but keep it to a Tools-badged model. See [`README.md`'s "Model choice for tool-calling"](../README.md#model-choice-for-tool-calling-read-this-before-the-proxy-section) section for the full finding, an important open caveat about multi-turn reliability, and the tool-call proxy as a fallback for models without native tool support.
...
**Correction, 2026-08-20:** the original framing here — "a reliability gap in Ollama's OpenAI-compatible tool-calling layer," fixed by the tool-call proxy — was too broad and pointed at the wrong layer. Re-tested `qwen2.5-coder:7b` directly against Ollama's *native* `/api/chat`, no OpenAI-compat translation involved, and got the identical failure (raw JSON text, not a structured call). The model's chat template genuinely supports `<tool_call>` markup and Ollama reports `"tools"` in its capabilities for this model — the plumbing is fine. The weights just don't reliably emit that exact wrapper. `qwen3-coder:30b`'s earlier failure in this same investigation (the malformed-schema/hallucinated-`fetch`-tool tangent) has not been root-caused — a later re-test of the same narrow single-tool-call scenario passed 3/3, live, both via curl and the real opencode CLI with no proxy involved, but that doesn't explain the earlier failure away. Treat `qwen3-coder:30b` as *inconsistent*, not fixed: reliable on some runs, not on others, cause unconfirmed.

**So: pick a Tools-badged model first (this session's opencode config now defaults to `qwen3-coder:30b`), the tool-call proxy is a fallback for models that don't have one, not the primary fix.** One important open caveat, found in a later full-session test: `qwen3-coder:30b` correctly executed a file read, then attempted an *unprompted* file-write using malformed tool-call markup opencode didn't recognize (it printed as text, nothing was actually written — confirmed via `git status`). Single controlled tool calls are proven reliable on this model; a full multi-turn tool surface is not yet proven reliable. Don't treat this as fully solved.

**Re-run the same test with a Tools-badged model to see the difference yourself:**

```powershell
opencode run "Read cv.md and summarize the target role in 2 sentences." --model ollama/qwen3-coder:30b
```
...
- Tool calls leaking into the chat response as raw JSON text: your `model` isn't a Tools-badged one (see above) — switch models before reaching for the tool-call proxy. If you're intentionally using a non-Tools-badged model, run `ai-tool-proxy-start` and point `baseURL` at `12347` instead of `12345`.
