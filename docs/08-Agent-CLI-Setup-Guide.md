...
**Correction, 2026-08-20 / ADR-014:** this was originally blamed on "Ollama's OpenAI-compat tool-calling layer." Re-tested directly against Ollama's *native* `/api/chat` (no OpenAI-compat translation at all) and got the identical failure, so that explanation was wrong. `qwen2.5-coder:7b`'s own chat template genuinely defines `<tool_call>` markup and Ollama's `/api/tags` lists `"tools"` in its capabilities — the template supports tool calls syntactically. The model's weights just aren't reliably trained to consistently emit that exact wrapper, so it falls back to printing its best-guess JSON as plain text. `qwen3-coder:30b` passed 3/3 on the same *single-call* probe and then failed **0/6** on the real multi-turn repair-loop contract — do not treat a larger Tools-badged Ollama model as an agentic fix. See `README.md`'s "Model choice for tool-calling" section and ADR-014. This `qwen2.5-coder:7b` example is the **known failure**, not a setup check to keep retrying.
...
**Do not treat Ollama's "Tools" badge as an agentic-ready default — this matters as much as the port.** `qwen2.5-coder:7b` reproducibly fails opencode's tool-calling (see the reproduced failure below). `qwen3-coder:30b` passed the same *single-call* probe 3/3 and then failed **0/6** on the real multi-turn repair-loop contract; `devstral-small-2:24b` never emitted structured tool_calls at the wire. Check `ollama.com/library/<model>` for the badge if you want, but it is editorial, not a verdict. The template points `provider.ollama.options.baseURL` at Ollama (`http://127.0.0.1:12345/v1` — use whatever `ai-port` shows you) and defaults `model` to `ollama/qwen3-coder:30b` as a **connection/chat default**, not an agentic-ready default. See [`README.md`'s "Model choice for tool-calling"](../README.md#model-choice-for-tool-calling-read-this-before-the-proxy-section) and ADR-014. The `qwen2.5-coder:7b` file-read example below is the **known failure**, not a setup check to keep retrying.
...
**So: the template's chat/connection default is `ollama/qwen3-coder:30b`, not an agentic-ready default.** The tool-call proxy is an opt-in translation layer for malformed native markup, not the primary fix and not a way to complete a multi-turn coding loop. Open caveat, found in a later full-session test: `qwen3-coder:30b` correctly executed a file read, then attempted an *unprompted* file-write using malformed tool-call markup opencode didn't recognize (it printed as text, nothing was actually written — confirmed via `git status`). Isolated single calls can pass; the repair-loop contract on this model is **0/6**. Don't treat this as solved. Don't re-run the file-read test expecting a Tools badge to change the agentic outcome.
...
- Tool calls leaking into the chat response as raw JSON text: the model is narrating a tool instead of calling one (the known `qwen2.5-coder:7b` failure). Switching to another Tools-badged Ollama model is **not** a proven agentic fix (ADR-014). If you specifically need a translation layer in front of malformed native markup, run `ai-tool-proxy-start` and point `baseURL` at `12347` instead of `12345` — that still is not a multi-turn coding-loop fix.
