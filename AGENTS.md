...
- Unsloth Dynamic 3.0 ladder (ADR-018): **UD-Q3_K_XL at context 8192 is the only quant that may inherit the recorded 3/3, and only when the vendor is NVIDIA and the card is 16 GB class**. Smaller free VRAM steps down (`UD-IQ3_XXS` / `UD-Q2_K_XL`). Larger free VRAM steps up (`UD-Q4_K_XL` and above) as **candidate-only** — the profile exists, and it does not inherit the 3/3. If the 8192 floor misses but the half-context floor holds, the same quant starts at 4096 and still does not inherit. If VRAM is too small but **system RAM can hold the GGUF**, `ai-agent-start` mmap's it on CPU (`--n-gpu-layers 0`). Tiny RAM (under ~10 GB free) still refuses. Do not sum multi-GPU VRAM. Do not partial-offload.
