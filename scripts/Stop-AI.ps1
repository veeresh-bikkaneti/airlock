...
# ------------------------------------------------------------
# 1️⃣ Identify and kill any running Ollama processes (UI app + server)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
if ($CleanFirewall) {
    $rules = Get-NetFirewallRule -DisplayName "AI-Platform-Ollama-Block-*" -ErrorAction SilentlyContinue
    if ($rules) {
