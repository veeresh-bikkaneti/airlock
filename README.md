...
**Correction, 2026-08-20:** an earlier version of this section, and of `docs/08-Agent-CLI-Setup-Guide.md`, blamed this on "Ollama's OpenAI-compatible tool-calling layer." That's wrong — re-tested directly against Ollama's *native* `/api/chat` (no OpenAI-compat translation involved at all) and got the identical failure. `qwen2.5-coder:7b`'s own chat template *does* define real `<tool_call>` markup (`ollama show qwen2.5-coder:7b --template`, or `/api/show`) and Ollama's `/api/tags` even lists `"tools"` in its capabilities. The template supports tool calls syntactically; the model's weights just aren't reliably trained to consistently emit that exact wrapper, so it falls through to printing its best-guess JSON as plain text instead. `ollama.com/library/<model>`'s "Tools" badge (a curated, editorial signal, not a technical one) turned out to be a decent practical predictor of this in testing, but the mechanism is model-training reliability, not a missing feature.

**The bigger fix, live-verified, is picking a model that's actually reliable at this — before reaching for the proxy below.** `qwen3-coder:30b` (already in `ai-models` on this box) passed 3/3 clean on direct, single-tool-call native API tests where `qwen2.5-coder:7b` failed 3/3. Point `provider.ollama.options.baseURL` straight at Ollama (`12345`, not the proxy) and set `model` to a model with the "Tools" badge on its Ollama library page first — `config/opencode.json.template` now defaults this way.

**Open, unresolved caveat — don't treat this as fully solved:** a live full-session opencode test against `qwen3-coder:30b` (not just the narrow single-tool-call tests above) still misbehaved on a later turn — it correctly executed a file read, then attempted an *unprompted* file-write using malformed, non-Hermes tool-call markup that opencode didn't recognize, so it printed as text instead of executing (confirmed via `git status` — nothing was actually written). Nothing was lost, but this shows the multi-turn, full-tool-surface case is still not proven reliable, only the narrow single-call case is. Treat local tool-calling as "usably better with the right model," not "solved."

Separately, Airlock's own model auto-selection (`ai-start`'s discovery step) picks purely on GPU-memory fit with no awareness of tool-calling reliability at all — it defaulted to `qwen2.5-coder:7b` on a fresh run today. Not yet fixed; flagged here so it doesn't get lost.

#### Tool-Call Proxy

A translation layer (`tool-proxy/`, port `12347`) that grammar-constrains Ollama's own `/api/chat` output so a model can't emit a malformed tool call, then re-shapes the result into a real OpenAI `tool_calls` response. **Opt-in fallback, not the default** — try a Tools-badged model on plain Ollama first (above). Useful if you specifically need a model without reliable native tool-calling. Start with `ai-tool-proxy-start`. Requests with no `tools` declared pass straight through unchanged either way.
