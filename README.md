...
`config/opencode.json.template` defaults `model` to `ollama/qwen3-coder:30b` as a **connection/chat default**, not an agentic-ready default. There is no passing agentic path on this branch for Ollama or vLLM. The one live 3/3 pass is Unsloth Qwen3.8-27B via llama-server + Pi on `feature/adr-013-pi-harness-verification` (ADR-013, not merged here).
